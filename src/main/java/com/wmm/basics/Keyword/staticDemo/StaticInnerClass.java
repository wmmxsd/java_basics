...
public class StaticInnerClass {
    ...
    public static void main(String[] args) {
        ...
        // new一个外部类
        StaticInnerClass oc1 = new StaticInnerClass();
        // 通过外部类的对象new一个非静态的内部类
        StaticInnerClass.InnerClass no_static_inner = oc1.new InnerClass();
        // 调用非静态内部类的方法
        System.out.println(no_static_inner.getKey());

        // 调用静态内部类的静态变量
        System.out.println(StaticInnerClass.InnerStaticClass.static_value);
        // 不依赖于外部类实例,直接实例化内部静态类
        StaticInnerClass.InnerStaticClass inner = new StaticInnerClass.InnerStaticClass();
        // 调用静态内部类的非静态方法
        System.out.println(inner.getValue());
        // 调用内部静态类的静态方法
        System.out.println(StaticInnerClass.InnerStaticClass.getMessage());
    }
